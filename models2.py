...
class GalaxyModel():
    ...
    def __init__(self, 
                 # Initial conditions
                 z_ini, Mh_ini, wechsler_ac,
                 # Parameters: metallicity
                 Z_in = 2E-5, yZ = 0.069,
                 # Parameters: MAH model
                 model_mah = 'neistein08', 
                 # Parameters: reionization model
                 model_reion = 'T_min', z_reion = 9.0, T_min_before = 7000.0, T_min_after = 20000.0, alpha1 = 1.0, 
                 # Alternative reiionization model
                 M_c1_early = 1E7, M_c1_0 = 2E10, 
        ...
        self.T_min_before = T_min_before
        self.T_min_after = T_min_after
        self.alpha1 = alpha1
    ...
    def dMh_dt(self, t, Mx_cur):
        ...
        #elif self.model_mah == 'wechsler02':
            #z = self.cosmo.age(t, inverse = True)
            #dz_dt = self.cosmo.age(t, derivative = 1, inverse = True)
            #alpha = self.alpha3
            #ret = - alpha * self.Mx_ini[IH] * np.exp(alpha * (self.z_ini-z)) * dz_dt

        elif self.model_mah == 'wechsler02':
            dz_dt = self.cosmo.age(t, inverse = True, derivative = True)
            ret = -2.0 * self.wechsler_ac * Mx_cur[IH] * dz_dt

        elif self.model_mah == 'wechslerv2':
            z = self.cosmo.age(t, inverse = True)
            dz_dt = self.cosmo.age(t, derivative = 1, inverse = True)
            cvir = concentration.concentration(Mx_cur[IH],'vir',z) 
            ac = np.random.normal(4.1/cvir, 1.0/cvir,1)[0]
            alpha4 = 2 * ac
            ret = - alpha4 * self.Mx_ini[IH] * np.exp(alpha4 * (self.z_ini-z)) * dz_dt
    ...
    def eps_g1(self, t, Mx_cur):
        ...
        elif self.model_reion in ['T_min', 'okamoto08']:

            z = self.cosmo.age(t, inverse = True)
            
            if self.model_reion == 'T_min':
                if z > self.z_reion:
                    T = self.T_min_before
                else:
                    T = self.T_min_after
                M_c1 = theory.MvirFromTvir(T, z)
